...
% centroid = 0.5*( nodes.position( springs.nodes(:,1) ,:) + nodes.position( springs.nodes(:,2) ,:) ) ;
% text( ...
% 	centroid(:,1) , ...
% 	centroid(:,2) , ...
% 	strtrim(cellstr(num2str((0:(size(centroid,1)-1))'))) , ...
% 	'HorizontalAlignment' , 'center' , ...
% 	'VerticalAlignment' , 'middle' )

% for pp = 1 : size(nodes.position,1)
% 	plot( ...
% 		nodes.position(pp,1) + 10*[0,nodes.force(pp,1)] , ...
% 		nodes.position(pp,2) + 10*[0,nodes.force(pp,2)] , ...
% 		'b-' , ...
% 		'LineWidth' , 1 )
% end
