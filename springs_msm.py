...
def main(argv):
	...
	for force_min in np.arange(0.000,0.800,0.200):
		for force_delta in np.arange(0.025,0.100,0.025):
			...
			if using_forcing_profile:
				...
				# force_min = 0.625 ; force_max = 0.675
				time_cycle = 5.0
				num_forces = 2
				num_cycles = 200
				iter_total = 0
				...
				if ( Path('.')/'..'/save_folder_name ).exists():
					...
					bio.display(lung,
						...
						ax_lims=ax_lims,
						delay=None,
						save_file_name=Path('.')/'..'/'breath_{:03d}.png'.format(0),
						show=False)
					...
					for iter_cycle in range(num_cycles):
						...
						if (iter_cycle+1) % 5 is 0:
							bio.display(lung,
								...
								ax_lims=ax_lims,
								delay=None,
								save_file_name=Path('.')/'..'/'breath_{:03d}.png'.format(iter_cycle+1),
								show=False)
